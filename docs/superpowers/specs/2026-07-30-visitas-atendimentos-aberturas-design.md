...
> **Errata 5 (2026-07-31):** Login de admin não conseguia carregar a tela de Aberturas (RPC estourando o `statement_timeout=8s` do role `authenticated`). Root cause: `_dashboard_rep_visible(sistema, id_representante, filtro)` era chamada como predicado por linha dentro da CTE `base` de `get_clientes_aberturas`, `get_clientes_aberturas_detalhe` e `get_representantes_abertura` — e o corpo da função faz `select role from profiles where id = auth.uid()` internamente. Como os argumentos da função dependem das colunas da linha (`m.sistema`, `m.id_representante`), o Postgres não conseguia hastear essa subconsulta pra fora do loop (mesma classe de bug da Errata 3 e do incidente de `vw_representantes_canonicos`): pra um representante (RLS restringe a poucas linhas) passava rápido, mas pra admin — sem restrição, ~75 mil linhas de `mv_clientes_pedidos` — cada uma das ~75 mil chamadas repetia a consulta a `profiles`, custando sozinho ~1,3s só nesse filtro (medido via `EXPLAIN ANALYZE`). Corrigido reescrevendo as 3 RPCs para calcular o papel do usuário (`profiles.role`) e as contas visíveis (`representante_contas`) **uma única vez**, em CTEs próprias (`meu_role`, `minhas_contas`) fora do loop por linha — o Postgres consegue promover essas subconsultas não-correlacionadas a `InitPlan`/`SubPlan`, executadas 1x por statement em vez de 1x por linha. `get_clientes_aberturas_detalhe` (admin, range de 1,5 ano): 2,84s → 0,33s. Testado também simulando login de representante (`set_config('request.jwt.claims', ...)`) para confirmar que o RLS continua restringindo corretamente ao portfólio dele. **Pendência conhecida (não corrigida agora, fora do escopo desta tela):** o mesmo padrão de chamada por linha de `_dashboard_rep_visible` existe em outras RPCs de Clientes (Atividade, Frequência, Curva ABC, Avaliação, Histórico de Compras, Agenda) — hoje não relatado como quebrado porque a base filtrada é menor nesses casos, mas sujeito ao mesmo risco conforme a base de clientes crescer.
>
> **Errata 4 (2026-07-30):** Correção da Errata 2 — a exclusão de "FUNCIONARIOS" havia sido aplicada em `vw_pedidos_v2`, a view base compartilhada por vendas (`mv_dashboard_kpis_diario`) e por identificação de clientes (`mv_clientes_pedidos`, `mv_dashboard_clientes_diario`). Isso zerou incorretamente o faturamento desses pedidos no Dashboard — usuário esclareceu que pedidos de funcionários são vendas legítimas para a empresa (só não devem contar como "cliente"). Corrigido: exclusão de `representante = 'FUNCIONARIOS'` revertida em `vw_pedidos_v2` (revenue voltou a incluir esses pedidos: R$47.885,28 / 523 pedidos restaurados) e movida para dentro de `mv_clientes_pedidos` e `mv_dashboard_clientes_diario` (recriadas), que são especificamente sobre identificação/contagem de clientes. `mv_dashboard_kpis_diario` (faturamento/pedidos) permanece sem essa exclusão.
>
